...
class AcademicYearController extends Controller
{
    ...
    /**
     * Get academic years for schedule history
     * Only returns academic years and semesters before the current active ones
     */
    public function getAcademicYearsHistory()
    {
        // First, get the current active academic year and semester
        $activeYear = AcademicYear::where('is_active', true)->first();
        $activeSemester = ActiveSemester::where('academic_year_id', $activeYear->academic_year_id)
            ->where('is_active', true)
            ->first();

        // Fetch and filter academic years
        $academicYears = AcademicYear::join('active_semesters', 'academic_years.academic_year_id', '=', 'active_semesters.academic_year_id')
            ->join('semesters', 'active_semesters.semester_id', '=', 'semesters.semester_id')
            ->select(
                'academic_years.academic_year_id',
